    ...
    if (last_status)
    {
      ...
      switch (cmd)
      {
        case 0:  // Note off
          if (data.size() >= 2)
          {
            events.emplace_back(Event::Type::note_off, chan, data[0], data[1]);
            data.clear();
          }
        break;
        ...
        case 1:  // Note on
          if (data.size() >= 2)
          {
            events.emplace_back(Event::Type::note_on, chan, data[0], data[1]);
            data.clear();
          }
        break;
        ...
        case 3:  // Control change
          if (data.size() >= 2)
          {
            events.emplace_back(Event::Type::control_change,
                                chan, data[0], data[1]);
            data.clear();
          }
        break;
